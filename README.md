...
- **Per-player reputation** on a -2..+2 scale (Blacklist / Negative / Neutral / Positive / Favorite)
...
- **Guild sync** - share blacklists/favorites with trusted guildmates (opt-in, trust-tiered)
- **Import/export** - compact shareable strings using LibDeflate
...
| Command | Description |
| --- | --- |
| `/rk` or `/repkeeper` | Toggle the main window |
| `/rk add Name-Realm [rep] [note]` | Add a player |
| `/rk remove Name-Realm` | Remove a player |
| `/rk note Name-Realm <text>` | Add a timeline note |
| `/rk tag Name-Realm <tagID>` | Toggle a tag |
| `/rk export` / `/rk import` | Open import/export dialogs |
| `/rk backup` | Create a manual backup |
| `/rk config` | Open settings |
